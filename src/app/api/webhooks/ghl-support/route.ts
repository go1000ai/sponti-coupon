...
export async function POST(request: NextRequest) {
  // Verify webhook secret — FAIL CLOSED: if the secret isn't configured,
  // reject rather than accepting unauthenticated callers.
  const secret = process.env.GHL_SUPPORT_WEBHOOK_SECRET;
  if (!secret) {
    console.error('[GHL Support Webhook] GHL_SUPPORT_WEBHOOK_SECRET is not configured — rejecting.');
