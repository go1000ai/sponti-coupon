    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900">{t('claimCallback.verifyingPayment')}</h2>
          <p className="text-gray-500 mt-2">{t('claimCallback.onlyMoment')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4">
        <div className="card p-8 max-w-md w-full text-center">
          <h2 className="text-xl font-bold text-red-600 mb-2">{t('claimCallback.somethingWentWrong')}</h2>
          <p className="text-gray-500 mb-4">{t('claimCallback.couldntVerify')}</p>
          <button onClick={() => router.push('/my-deals')} className="btn-primary">
            {t('claimCallback.goToMyDeals')}
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-4">
        <div className="card p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">{t('claimCallback.waitingConfirmation')}</h2>
          <p className="text-gray-500 mb-4">
            {t('claimCallback.paymentProcessing')}
          </p>
          <div className="bg-gray-50 rounded-lg p-4 mb-4">
            <Loader2 className="w-6 h-6 text-primary-500 animate-spin mx-auto" />
            <p className="text-sm text-gray-400 mt-2">{t('claimCallback.checkingStatus')}</p>
          </div>
          <button onClick={() => router.push('/my-deals')} className="btn-outline w-full">
            {t('claimCallback.checkLater')}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="card p-8 max-w-md w-full text-center">
        ...
        <h2 className="text-2xl font-bold text-green-600 mb-2">{t('claimCallback.dealClaimed')}</h2>
        <p className="text-gray-500 mb-6">{t('claimCallback.depositConfirmed')}</p>
        ...
        {claim?.deal?.expires_at && (
          <div className="bg-primary-50 rounded-lg p-4 mb-6">
            <p className="text-sm text-primary-600 font-medium mb-2">{t('claimCallback.redeemBefore')}</p>
